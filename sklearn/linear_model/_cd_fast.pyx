...
def enet_coordinate_descent(floating[::1] w,
                            ...
                            object rng, bint random=0, bint positive=0, bint twinkle=0, floating tw=1, bint minous=0, floating minous_tw=1,):
            ...
            if (w_max == 0.0 or
                ...
                if twinkle:
                    ...
                    for i in range(n_features):
                        Twinkle[i] = w[i] ** tw
                    l1_norm = _asum(n_features, &Twinkle[0], 1)
                else:
                ...
                if minous:
                    for i in range(n_features):
                        Twinkle[i] = w[i] ** minous_tw
                    l1_norm = l1_norm - _asum(n_features, &Twinkle[0], 1)
...
def enet_coordinate_descent_gram(floating[::1] w,
                                 ...
                                 bint random=0, bint positive=0, bint twinkle=0, floating tw=1):
    
    warnings.warn("enet_coordinate_descent_gram")
    """Cython version of the coordinate descent algorithm
        for Elastic-Net regression
        ...
        for n_iter in range(max_iter):
            ...
            if w_max == 0.0 or d_w_max / w_max < d_w_tol or n_iter == max_iter - 1:
                ...
                if twinkle:
                    for i in range(n_features):
                        Twinkle[i] = w[i] ** tw
                    gap += (alpha * _asum(n_features, &Twinkle[0], 1) - const * y_norm2 +  const * q_dot_w + 0.5 * beta * (1 + const ** 2) * w_norm2)
                else:
                    gap += (alpha * _asum(n_features, &w[0], 1) - const * y_norm2 +  const * q_dot_w + 0.5 * beta * (1 + const ** 2) * w_norm2)
...
def enet_coordinate_descent_multi_task(
        ...
        int max_iter, floating tol, object rng, bint random=0, bint twinkle=0, floating tw=1):
    """Cython version of the coordinate descent algorithm
    ...
    warnings.warn("enet_coordinate_descent_multi_task")
    if l1_reg == 0:
                ...
                if twinkle:        
                    for ii in range(n_features):
                        ...
                        for t in range(n_tasks):
                            Twinkle[t, ii] = W[t, ii] ** tw
                        l21_norm += _nrm2(n_tasks, &Twinkle[0, ii], 1)
